...
class CameraManager:
    def __init__(self):
        ...
        self.frame_queue = queue.Queue(maxsize=2)  # Limit queue size to reduce lag
        self.capture_thread = None
        
    def start_camera(self):
        if self.cap is None:
            self.cap = cv2.VideoCapture(0)
            # Optimize camera settings to reduce lag
...
def video_feed():
    def generate_frames():
        ...
        frame_count = 0
        process_every_n_frames = 3  # Process face recognition every 3rd frame to reduce lag
        ...
        while camera_manager.is_running:
            ...
            if current_mode == 'attendance' and known_encodings and frame_count % process_every_n_frames == 0:
                ...
                face_locations = face_recognition.face_locations(rgb_frame, model="hog")  # Use HOG model for speed
            ...
            # Encode frame as JPEG with compression to reduce bandwidth
            encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 85]  # Adjust quality to balance size/quality
...
def camera_status():
    """New endpoint to check camera status"""
    return jsonify({
